# Code used in R for downstream data analyses
...
keyfile <- keyfile %>% filter(!Sample_ID %in% "LMM_12")
analysis.name <- "DE_Gall_tissues_wo_outlier"
...
find_de_edger <- function (dge,paths) {
  dge <- calcNormFactors(dge, method="TMM")
  design <- eval(parse(text = paste0('model.matrix(~0 + ',
                                     group,
                                     ', data = keyfile)')))
  colnames(design) <- levels(keyfile[, which(colnames(keyfile) == group)])
  v <- voom(dge,design,plot=TRUE)
  fit <- lmFit(v,design)
  fit <- eBayes(fit)
}

de_edger_mds <- function (dge, ) {
